...
Do {
    Do {
        ...
        Add-Content -Path $LogFile -Value ((Get-Date).ToString("yyyy-MM-dd HH:mm:ss")+" - CPU Usage =  "+$cpuUsage.ToString)
        Start-Sleep -Seconds 5
    } while ($cpuUsage -gt $max_cpu)

    $files = Get-ChildItem -Path $location | Select-Object -Last 50
    Add-Content -Path $LogFile -Value ((Get-Date).ToString("yyyy-MM-dd HH:mm:ss")+" - Found "+$files.count.ToString()+" files to move")
    foreach($file in $files) {
        $NewFileLocation = $destination+"\"+$file.Name
        Add-Content -Path $LogFile -Value ((Get-Date).ToString("yyyy-MM-dd HH:mm:ss")+" - Moving "+$file.Fullname+"  to "+ $NewFileLocation)
