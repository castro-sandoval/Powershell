...
$EmailList        = "sandoval.castroneto@coupa.com"
...
if (Test-Path -Path $TSQL_restoreFile) {
            ...
            if ($dbName.Contains("?")) {  # Check database name with special characters not exported correctly by sqlcmd.exe
                ...
                if (Test-Path -Path $filePath) {
                    ...
                    if ($sqlRestorecmd.Contains(" RECOVERY")) {
                        ...
                        Invoke-Sqlcmd -Query $checkdbCmd -ServerInstance $ServerInstance -Username "UUUUUU" -Password "**" -Database "master" -Verbose 4> $VerboseOutput
    ...
    $cmd = "EXEC msdb.dbo.sp_send_dbmail @profile_name='BackupMonitor', @recipients = '"+$EmailList+"', @body = 'The restoration process failed on the master server MSX. Check attached log file.', @subject = 'Backup ALERT - Restauration test found errors on server "+$ServerInstance+"', @file_attachments='"+$LogFile+"'"
}
